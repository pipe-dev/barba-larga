...
import { services, Service, getBaseAvailableTimes } from "@/lib/data";
...
function BookingForm({ onNavigate }: { onNavigate: (scene: Scene) => void }) {
  ...
  const chosenServices = React.useMemo(() => services.filter(s => selectedServices.includes(s.id)), [selectedServices]);
  const chosenBarber = React.useMemo(() => team.find(b => b.id === selectedBarberId), [selectedBarberId, team]);
  
  const { totalPrice, totalDuration } = React.useMemo(() => {
    const initial = { totalPrice: 0, totalDuration: 0 };
    return chosenServices.reduce((acc, service) => {
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between gap-4 p-3 bg-muted/50 rounded-md">
                          <div className="flex items-center gap-3">
                            <Image src={chosenBarber.imageUrl} alt={chosenBarber.name} width={40} height={40} className="rounded-full" />
                            <div>
                          ...
                          <div key={service.id} className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                                  ...
                                  {service.mediaType === 'video' ? (
                                      ...
                                      <Image src={service.mediaUrl} alt={service.name} width={64} height={64} className="object-cover h-full w-full" />
                                  )}
                                  </div>
                                  <div>
