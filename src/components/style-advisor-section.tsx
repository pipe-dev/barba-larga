...
import { aiStyleAdvisor, AIStyleAdvisorOutput } from "@/ai/flows/ai-style-advisor";
...
export function StyleAdvisorSection({ onNavigate }: { onNavigate: (scene: Scene) => void }) {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    setError(null);
    cancel(); // Detiene cualquier audio anterior
    try {
  ...
  const handleBookRecommended = () => {
    if (recommendation?.suggestedServiceIds) {
      ...
      toast({
        ...
        description: "Los servicios recomendados se han añadido a tu selección. ¡Continúa para reservar!",
      });
      onNavigate('booking');
    }
  };
